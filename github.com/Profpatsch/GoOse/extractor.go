...
func (extr *ContentExtractor) getTitleUnmodified(article *Article) string {
	title := ""
	doc := article.Doc
...
// extract the title, apply delimiter heuristics, and trim
func (extr *ContentExtractor) getTitle(article *Article) string {
	title := extr.getTitleUnmodified(article)
